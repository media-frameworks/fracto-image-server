...
export const raster_fill = async (
   ...
   console.log('raster_fill', level_data_sets.length)
   ...
   for (let canvas_x = 0; canvas_x < width_px; canvas_x++) {
      if (canvas_x % 100 === 0) {
         console.log(`${Math.round(10000 * canvas_x / width_px) / 100}%`)
      }
      if (update_callback) {
   ...
   const bad_tiles_count = Object.keys(BAD_TILES)
   if (bad_tiles_count) {
      console.log('bad_tiles_count', bad_tiles_count)
   }
}