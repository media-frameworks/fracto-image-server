...
export const get_tiles = (width_px, focal_point, scope, aspect_ratio) => {
  ...
  for (let level = min_level; level < max_level; level++) {
    const level_tiles = tiles_in_scope(level, focal_point, scope, aspect_ratio);
    if (level_tiles.length) {
      all_tiles.push({ level, level_tiles });
    }
...
const tiles_in_scope = (level, focal_point, scope, aspect_ratio = 1.0, set_name = TILE_SET_INDEXED) => {
  ...
  if (!set_level || !set_level.columns.length) return [];
  // Filter columns in a single pass
  const short_codes = [];
  const max_y = viewport.top > Math.abs(viewport.bottom) ? viewport.top : Math.abs(viewport.bottom);
...
export const init_canvas_buffer = (width_px, aspect_ratio) => {
  ...
  for (let x = 0; x < width_px; x++) {
    // Each pixel: [pattern, iteration] as Int16Array
    buffer[x] = new Array(height_px);
    for (let y = 0; y < height_px; y++) {
      buffer[x][y] = new Int16Array([0, 4]);
    }
  }
  return buffer;
};
...
export const fill_canvas_buffer = async (
  ...
  const all_level_sets = get_tiles(width_px, focal_point, scope, aspect_ratio);
  if (update_callback) {
  ...
  const level_data_sets = all_level_sets
    .map((level_set) => {
      ...
      const tile_width = level_set.level_tiles[0].bounds.right - level_set.level_tiles[0].bounds.left;
      level_set.tile_increment = tile_width / 256;
      return level_set;
    })
    .sort((a, b) => b.level - a.level);
  let tile_index = 0;
  for (let i = 0; i < level_data_sets.length; i++) {
    const level_set = level_data_sets[i];
    for (let j = 0; j < level_set.level_tiles.length; j++) {
      const tile = level_set.level_tiles[j];
      tile_index++;
      if (BAD_TILES[tile.short_code]) continue;
      if (update_callback) {
...
export const raster_fill = async (
  ...
  console.log('raster_fill')
  if (!canvas_buffer) return;
  const canvas_increment = scope / width_px;
  const height_px = Math.round(width_px * aspect_ratio);
  // Precompute scales for memory efficiency
  const horz_scale = new Float64Array(width_px);
  for (let horz_x = 0; horz_x < width_px; horz_x++) {
    horz_scale[horz_x] = focal_point.x + (horz_x - width_px / 2) * canvas_increment;
  }
  const vert_scale = new Float64Array(height_px);
  ...
  for (let canvas_x = 0; canvas_x < width_px; canvas_x++) {
    ...
    for (let canvas_y = 0; canvas_y < height_px; canvas_y++) {
        ...
        for (let t = 0; t < level_data_set.level_tiles.length; t++) {
          ...
          if (
            ...
            try {
              ...
              if (tile_data && tile_data[tile_x]) {
                canvas_buffer[canvas_x][canvas_y][0] = scalar * tile_data[tile_x][tile_y][0];
                canvas_buffer[canvas_x][canvas_y][1] = scalar * tile_data[tile_x][tile_y][1];
              } else {
                canvas_buffer[canvas_x][canvas_y][0] = 0;
                canvas_buffer[canvas_x][canvas_y][1] = 4;
              }
              found_point = true;
            } catch (e) {
              if (!tile_data) {
      ...
      if (!found_point && out_of_bounds && canvas_buffer && canvas_buffer[canvas_x] && canvas_buffer[canvas_x][canvas_y]) {
        const { pattern, iteration } = FractoFastCalc.calc(x, y);
        canvas_buffer[canvas_x][canvas_y][0] = pattern;
        canvas_buffer[canvas_x][canvas_y][1] = iteration;
      }
    }
  }
